...
router.push(
  `/FirstGeneration?prompt=${encodeURIComponent(prompt)}&name=${name}&count=${count}`
);
  };

  return (
    <div className="min-h-screen bg-white px-8 py-6 mt-[65px]">
      {/* Header */}
            ...
            {uploadedImages.length > 0 && (
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {uploadedImages.map((img) => (
                  <div key={img.id} className="relative group w-1/2 aspect-square rounded border">
